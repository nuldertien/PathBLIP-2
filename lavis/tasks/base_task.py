...
class BaseTask:
    ...
    def build_datasets(self, cfg):
        ...
        # Following code is added to make use of paths that are relative to the folder where the config file is located
        BASE_DIR = Path().resolve()
        
        def adjust_paths(data, base_dir):
            if isinstance(data, (dict, DictConfig)):
                for key, value in data.items():
                    if isinstance(value, (dict, DictConfig)):
                        # Recursively process nested dictionaries
                        adjust_paths(value, base_dir)
                    elif key in ["url", "storage"]:
                        # Modify the value by prepending BASE_DIR
                        data[key] = str(base_dir / value)

        for name in datasets_config:
            dataset_config = datasets_config[name]
            adjust_paths(dataset_config, BASE_DIR)

...
class RunnerBase:
    ...
    def optimizer(self):
        if self._optimizer is None:
            ...
            param_count_by_module = defaultdict(int)

            for name, param in self._model.named_parameters():
                if param.requires_grad:

                    module_key = ".".join(name.split('.')[:1])  
                    param_count_by_module[module_key] += param.numel()

            print("Trainable parameters by top-level sub-module:")
            for module_name, count in param_count_by_module.items():
    ...
    @property
    def save_freq(self):
        save_freq = self.config.run_cfg.get("save_freq", 10)
    ...
    def train(self):
        ...
        self.log_stats(test_results['test'], "test")

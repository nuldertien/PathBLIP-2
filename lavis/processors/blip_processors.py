...
@registry.register_processor("pathology_caption")
class PathologyCaptionProcessor(BaseProcessor):
    def __init__(self, prompt="", max_words=1024):
        self.prompt = prompt
        self.max_words = max_words
    ...
    def from_config(cls, cfg=None):
        ...
        prompt = cfg.get("prompt", "")
        max_words = cfg.get("max_words", 1024)

        return cls(prompt=prompt, max_words=max_words)

    def pre_caption(self, caption):
        # already did processing the text before so no additional processing 
        # is needed.
        # Only thing need to think about, if capital letters are allowed or 
        # not
...
class BlipQuestionProcessor(BaseProcessor):
    def __init__(self, max_words=512):
        self.max_words = max_words
